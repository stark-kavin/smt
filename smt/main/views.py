...
def display_invoice_view(request, invoice_id):
    ...
    all_invoice = Invoice.objects.filter(party=invoice.party)
    total_pending = 0
    for i in all_invoice:
        if i.total and i.paid:
            total_pending += float(i.total) - float(i.paid)
        elif i.total and not i.paid:
            total_pending += float(i.total)
        elif not i.total and i.paid:
            total_pending += float(i.paid)
        else:
            total_pending += 0
    invoice.total_pending = str(total_pending)
